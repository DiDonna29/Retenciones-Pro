  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full">
        ...
        <Card className="glass-card overflow-hidden border-none shadow-2xl relative">
          ...
          <CardContent className="space-y-8 p-6">
            <div className="space-y-4">
              <Label htmlFor="gross-amount" className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/60">
                {t("grossAmount")}
              </Label>
              <div className="relative group">
                ...
                <Input
                  ...
                  className="pl-14 h-20 text-3xl font-black bg-muted/20 border-none focus-visible:ring-4 focus-visible:ring-primary/20 rounded-3xl transition-all"
                  value={grossAmount}
      ...
      <div className="lg:col-span-7 space-y-8">
            ...
            <CardContent className="p-10 relative z-10">
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <h2 className="text-5xl md:text-7xl font-black mt-3 tabular-nums tracking-tighter leading-none">
                    {formatCurrency(calculations.net)}
                  </h2>
                </div>
                <div className="p-6 bg-white/15 rounded-3xl backdrop-blur-xl">
                  <Wallet className="w-10 h-10" />
                </div>
              </div>
              <div className="flex items-center gap-3 text-xs font-black bg-black/10 w-fit px-6 py-3 rounded-full">
                <ReceiptText className="w-4 h-4" />
                <span>INVOICE PREVIEW READY</span>
              </div>
            </CardContent>
          </Card>

          <Card className="border-none shadow-xl glass-card overflow-hidden group">
            <CardContent className="p-8">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-xs font-black text-muted-foreground uppercase tracking-[0.2em]">{t("totalTaxes")}</p>
                  <h2 className="text-3xl font-black mt-2 tabular-nums text-destructive tracking-tight">
            ...
            <CardContent className="p-8">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h2 className="text-3xl font-black mt-2 tabular-nums text-primary tracking-tight">
        ...
        >
          <Card className="border-none shadow-xl glass-card min-h-[350px]">
            <CardHeader>
              <CardTitle className="text-sm font-black flex items-center gap-3 tracking-widest uppercase opacity-60">
            ...
            <CardContent className="h-[250px] relative">
              {calculations.gross > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-muted-foreground/40 space-y-4">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">{t("placeholderAmount")}</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-none shadow-xl glass-card min-h-[350px]">
            ...
            <CardContent className="space-y-5 pt-2">
              ...
              <div className="pt-6 border-t-2 border-primary/10 flex flex-col gap-1">
                <span className="font-black text-[10px] uppercase tracking-[0.3em] text-muted-foreground/60">{t("netAmount")}</span>
                <span className="font-black text-4xl text-primary tracking-tighter tabular-nums">
                  {formatCurrency(calculations.net)}
                </span>
              </div>
            </CardContent>
...
function SummaryItemRow({ label, value, isPositive, isNegative }: { label: string; value: string; isPositive?: boolean; isNegative?: boolean }) {
  return (
    <div className="flex justify-between items-center group">
      <span className="text-xs font-bold text-muted-foreground group-hover:text-foreground transition-colors uppercase tracking-widest">{label}</span>
      <span className={`text-sm font-black tabular-nums ${isPositive ? 'text-primary' : isNegative ? 'text-destructive' : 'text-foreground'}`}>
